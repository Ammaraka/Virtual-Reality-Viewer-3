...
BOOL check_for_card(const char* RENDERER, const char* bad_card)
{
	if (!strnicmp(RENDERER, bad_card, strlen(bad_card)))
	{
		std::string buffer = llformat(
			"Your video card appears to be a %s, which Angstrom does not support.\n"
			"\n"
			"Angstrom requires a video card with 32 Mb of memory or more, as well as\n"
			...
			"for support.\n"
			"\n"
			"You can try to run Angstrom, but it will probably crash or run\n"
