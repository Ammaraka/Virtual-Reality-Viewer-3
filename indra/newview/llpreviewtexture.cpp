...
void LLPreviewTexture::onSaveAsBtn(LLUICtrl* ctrl, void* data)
{
	...
	if (value == "format_jp2")
	{
		self->saveAs(LLPreviewTexture::FORMAT_JP2);
	}
	else if (value == "format_png")
	{
...
{
	saveAs(LLPreviewTexture::FORMAT_JP2);
}
...
void LLPreviewTexture::saveAs(EFileformatType format)
{
	...
	switch (format)
	{
		...
		case LLPreviewTexture::FORMAT_TGA:
			...
			break;
		default:
		case LLPreviewTexture::FORMAT_JP2:
			callback = LLPreviewTexture::onFileLoadedForSaveJP2;
			saveFilter = LLFilePicker::FFSAVE_JP2;
			break;
	}
...
// static
void LLPreviewTexture::onFileLoadedForSaveJP2(BOOL success, 
					   ...
					   void* userdata)
{
	...
	if( self && final && success )
	{
		...
		else if( !image_j2c->save( self->mSaveFileName ) )
		{
			LLSD args;
			args["FILE"] = self->mSaveFileName;
