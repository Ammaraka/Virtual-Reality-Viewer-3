...
BOOL LLPanelPeople::postBuild()
{
	...
	// <FS:Ansariel> Commented out because of FIRE-5184; Also missing in V3
...
void LLPanelPeople::updateNearbyList()
{
	...
	for (;pos_it != pos_end && item_it != item_end; ++pos_it, ++item_it )
	{
		...
		{
			// chat alerts
			if (RadarReportChatRange && (avRange <= LLWorld::getInstance()->getSayDistance()) && avRange > -1)
			{
				LLStringUtil::format_map_t args;
		...
		else 
		{
			...
			if (lastRadarSweep.count(avId) == 1) // normal case, check from last position
			{
				...
				{
					if ((avRange <= LLWorld::getInstance()->getSayDistance() && avRange > -1) && (rf.lastDistance > LLWorld::getInstance()->getSayDistance() || rf.lastDistance == -1))
					{
						LLStringUtil::format_map_t args;
						args["DISTANCE"] = llformat("%3.2f", avRange);
						std::string message = getString("entering_chat_range", args);
					...
					else if ((avRange > LLWorld::getInstance()->getSayDistance() || avRange == -1) && (rf.lastDistance <= LLWorld::getInstance()->getSayDistance() && rf.lastDistance > -1))
		...
		if (avRange > -1)
		{
			if (avRange <= LLWorld::getInstance()->getSayDistance())
			{
				radarRangeCell->setColor(LLUIColorTable::instance().getColor("AvatarListItemChatRange", LLColor4::red));
				inChatRange++;
			}
			else if (avRange <= LLWorld::getInstance()->getShoutDistance())
			{
	...
	for (std::multimap <LLUUID, radarFields>::const_iterator i = lastRadarSweep.begin(); i != lastRadarSweep.end(); ++i)
	{
		...
		if (!mNearbyList->contains(prevId))
		{
			...
			if (RadarReportChatRange && (rf.lastDistance <= LLWorld::getInstance()->getSayDistance()) && rf.lastDistance > -1)
