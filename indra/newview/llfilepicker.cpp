...
#define IMAGE_FILTER L"Images (*.tga; *.bmp; *.jpg; *.jpeg; *.png; *.jp2)\0*.tga;*.bmp;*.jpg;*.jpeg;*.png;*.jp2\0"
...
BOOL LLFilePicker::getSaveFile(ESaveFilter filter, const std::string& filename)
{
	...
	case FFSAVE_JP2:
		if (filename.empty())
		{
			wcsncpy( mFilesW,L"untitled.jp2", FILENAME_BUFFER_SIZE);
		}
		mOFN.lpstrDefExt = L"jp2";
		mOFN.lpstrFilter =
			L"Compressed Images (*.jp2)\0*.jp2\0" \
			L"\0";
		break;
	case FFSAVE_SCRIPT:
		if (filename.empty())
...
BOOL LLFilePicker::getSaveFile( ESaveFilter filter, const std::string& filename )
{
	...
	if (picker)
	{
		...
		case FFSAVE_JP2:
			caption += add_simple_mime_filter_to_gtkchooser
				(picker, "images/jp2",
				 LLTrans::getString("compressed_image_files") + " (*.jp2)");
			suggest_ext = ".jp2";
			break;
		case FFSAVE_SCRIPT:
